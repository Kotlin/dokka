...
import org.jetbrains.dokka.analysis.java.util.typeConstructorsBeingExceptions
...
import org.jetbrains.dokka.analysis.java.util.isGetterFor
import org.jetbrains.dokka.analysis.java.util.isSetterFor
import org.jetbrains.dokka.analysis.java.util.splitFunctionsAndAccessors
...
class DokkaPsiParser(
    ...
    private val isLightAnnotation: (PsiAnnotation) -> Boolean,
    private val isLightAnnotationAttribute: (JvmAnnotationAttribute) -> Boolean
) {
    ...
    private fun Collection<PsiAnnotation>.toListOfAnnotations() =
        filter { !isLightAnnotation(it) }.mapNotNull { it.toAnnotation() }
    ...
    private fun PsiAnnotation.toAnnotation(): Annotations.Annotation? {
        ...
        return psiReference?.let { psiElement ->
            Annotations.Annotation(
                ...
                params = attributes
                    .filter { !isLightAnnotationAttribute(it) }
