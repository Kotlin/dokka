...
fun DRI.Companion.from(psi: PsiElement) = psi.parentsWithSelf.run {
    val psiMethod = filterIsInstance<PsiMethod>().firstOrNull()
    val psiField = filterIsInstance<PsiField>().firstOrNull()
...
fun DriTarget.Companion.from(psi: PsiElement): DriTarget = psi.parentsWithSelf.run {
    return when (psi) {
        ...
        else -> filterIsInstance<PsiParameter>().firstOrNull()?.let {
            val callable = filterIsInstance<PsiMethod>().firstOrNull()
...
fun PsiElement.getKotlinFqName(): String? = this.kotlinFqNameProp

//// from import org.jetbrains.kotlin.idea.base.psi.kotlinFqName
val PsiElement.kotlinFqNameProp: String?
