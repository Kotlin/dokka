...
class JavaPsiDocCommentParser(
    ...
    private fun parseParamTag(
        ...
        val method = analysedElement as? PsiMethod ?: throw IllegalStateException("TODO [beresnev]")
    ...
    private fun wrapTagIfNecessary(tags: List<DocTag>): CustomDocTag {
        val isFile = (tags.singleOrNull() as? CustomDocTag)?.name == MarkdownElementTypes.MARKDOWN_FILE.name
        return if (isFile) {
            tags.first() as CustomDocTag
        } else {
            CustomDocTag(tags, name = MarkdownElementTypes.MARKDOWN_FILE.name)
        }
    }
