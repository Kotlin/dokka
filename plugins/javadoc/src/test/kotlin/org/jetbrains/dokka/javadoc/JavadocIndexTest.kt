...
internal class JavadocIndexTest : AbstractJavadocTemplateMapTest() {
    ...
    fun `handles correct number of elements`() {
        testIndexPages { indexPages ->
            assertEquals(39, indexPages.sumBy { it.elements.size })
        }
    }

    @Test
    fun `temlateMap for class index`() {
        testIndexPagesTemplateMaps { templateMaps ->
            val element = (templateMaps[2]["elements"] as List<TemplateMap>).first()
    ...
    fun `templateMap for function index`() {
        testIndexPagesTemplateMaps { templateMaps ->
            val element = (templateMaps[0]["elements"] as List<TemplateMap>)[1]
