...
class MultimodulePageCreator(
    ...
    override fun invoke(): RootPageNode {
        ...
        val commentsConverter = context.plugin(Processing::class)?.querySingle { commentsToContentTranslator }
        val signatureProvider = context.plugin(Processing::class)?.querySingle { signatureProvider }
                ...
                modules.mapNotNull { module ->
                    ...
                    paragraph?.let {
                        ...
                        val content = ContentGroup(
                            DocTagToContentTranslator.buildContent(it, dci, emptySet()),
                            dci,
                            emptySet(),
                            emptySet()
                        )
