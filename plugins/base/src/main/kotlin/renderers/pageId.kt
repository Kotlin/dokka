 ...
 */
private fun pageId(dri: DRI, sourceSets: Set<DisplaySourceSet>): String {
    return dri.classNames?.let { "${dri.copy(callable = null).shortenToUrl()}/${sourceSets.shortenToUrl()}" }
        ?: "${dri.shortenToUrl()}/${sourceSets.shortenToUrl()}"
}