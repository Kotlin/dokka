...
class SourceSetMergingPageTransformer(context: DokkaContext) : PageTransformer {
    ...
    private fun transformWithMergedSourceSets(
        ...
        return when (contentNode) {
            is ContentComposite -> contentNode.transformChildren(::transformWithMergedSourceSets)
            else -> run {
                val mergedSourceSets = contentNode.sourceSets.map { mergedSourceSets.getValue(it.key) }
                contentNode.withSourceSets(mergedSourceSets.toSet())
            }
        }
    }
}
