...
abstract class SamplesTransformer(val context: DokkaContext) : PageTransformer {
    ...
    private fun setUpAnalysis(context: DokkaContext) = context.configuration.sourceSets.map {
        it to AnalysisEnvironment(DokkaGenerator.DokkaMessageCollector(context.logger), it.analysisPlatform).run {
    ...
    private fun ContentNode.addSample(contentPage: ContentPage, platform: DokkaSourceSet, fqName: String, analysis: Map<DokkaSourceSet, EnvironmentAndFacade>): ContentNode {
