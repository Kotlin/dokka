...
internal class DokkaTestGenerator(
    ...
    fun generate() = with(testMethods) {
        ...
        val platforms: Map<DokkaSourceSet, EnvironmentAndFacade> = dokkaGenerator.setUpAnalysis(configuration)
        analysisSetupStage(platforms)

        val context =
            dokkaGenerator.initializePlugins(configuration, logger, platforms, pluginOverrides)
