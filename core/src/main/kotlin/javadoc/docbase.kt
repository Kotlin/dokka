...
// TODO tags
open class DocumentationNodeAdapter(override val module: ModuleNodeAdapter, node: DocumentationNode) :
    DocumentationNodeBareAdapter(node), HasModule {
    override fun inlineTags(): Array<out Tag> = buildInlineTags(module, this, node.content, module.formatService).toTypedArray()
    override fun firstSentenceTags(): Array<out Tag> = buildInlineTags(module, this, node.summary, module.formatService).toTypedArray()

    override fun tags(): Array<out Tag> {
        val result = ArrayList<Tag>(buildInlineTags(module, this, node.content, module.formatService))
        node.content.sections.flatMapTo(result) {
            when (it.tag) {
                ContentTags.SeeAlso -> buildInlineTags(module, this, it, module.formatService)
                else -> emptyList<Tag>()
            }
        }
        ...
        node.deprecation?.let {
            ...
            if (content != null) {
                result.add(TagImpl(this, "deprecated", content, module.formatService))
            }
        }
...
class AnnotationTypeDocAdapter(module: ModuleNodeAdapter, node: DocumentationNode)
    : ClassDocumentationNodeAdapter(module, node), AnnotationTypeDoc {
...
class ModuleNodeAdapter(val module: DocumentationModule, val reporter: DocErrorReporter, val outputPath: String,
                        val formatService: StructuredFormatService? = null) :
...
private fun DocumentationNodeAdapter.collectParamTags(kind: NodeKind, sectionFilter: (ContentSection) -> Boolean) =
        (node.details(kind)
                ...
                .map { ParamTagAdapter(module, this, it.name, true, it.content.children, module.formatService) }

                + node.content.sections
                .filter(sectionFilter)
                .map { ParamTagAdapter(module, this, it.subjectName ?: "?", true, it.children, module.formatService) })
